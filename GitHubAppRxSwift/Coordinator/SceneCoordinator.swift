...
class SceneCoordinator: NSObject, SceneCoordinatorType {
  ...
  @discardableResult
  func dismisModal(animated: Bool) -> Completable {
    
    let subject = PublishSubject<Void>()
     currentViewController.dismiss(animated: animated)
     self.currentViewController = SceneCoordinator.actualViewController(for: currentViewController.presentingViewController!)
     subject.onCompleted()
    return subject.asObservable()
    .take(1)
    .ignoreElements()
